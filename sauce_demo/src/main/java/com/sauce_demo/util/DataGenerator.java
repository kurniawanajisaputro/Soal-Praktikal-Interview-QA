...
//                System.out.println(isValid);
    ...
    public String datanoBpjs() {
        isValid = false;
        intLoop = 0;
        String noBpjs   = "";
        noBpjs = faker.number().digit();
        while(!isValid){
            try{
                noBpjs = faker.phoneNumber().phoneNumber();
                matcher = Pattern.compile("^(0|62|\\+62)\\d{9,15}$").matcher(noBpjs);
                isValid = matcher.find();
                if(intLoop==250){
                    System.out.println("SUDAH MENCOBA MEMBUAT DATA No HP SEBANYAK 15 KALI DAN GAGAL !!");
                    System.exit(1);
    ...
    public String datanoKTP() {
        ...
        intLoop = 0;
        String noKTP   = "";
        noKTP = faker.number().digit();
        while(!isValid){
            try{
                noKTP = faker.phoneNumber().phoneNumber();
                matcher = Pattern.compile("^(0|62|\\+62)\\d{9,15}$").matcher(noKTP);
                isValid = matcher.find();
                if(intLoop==250){
                    System.out.println("SUDAH MENCOBA MEMBUAT DATA No HP SEBANYAK 15 KALI DAN GAGAL !!");
                    System.exit(1);
